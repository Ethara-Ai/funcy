from contextlib import (
    ContextDecorator,
    contextmanager,
)  # reexport these for backwards compat
import functools
...
class Call(object):
    ...
    def __getattr__(self, name):
        try:
            res = self.__dict__[name] = arggetter(self._func)(
                name, self._args, self._kwargs
            )
            return res
    ...
    def __str__(self):
        ...
        args = ", ".join(
            list(map(str, self._args)) + ["%s=%s" % t for t in self._kwargs.items()]
        )
...
def has_1pos_and_kwonly(func):
    ...
    return (
        kinds[P.POSITIONAL_ONLY] + kinds[P.POSITIONAL_OR_KEYWORD] == 1
...
def arggetter(func, _cache={}):
    ...
    if original.__defaults__:
        defaults.update(
            zip(posnames[-len(original.__defaults__) :], original.__defaults__)
        )
